...
let handler = async(m, { conn, text, usedPrefix, command, isPrems }) => {
	...
	try {
		let anu = await (await fetch(`https://api.tiklydown.eu.org/api/download?url=${text}`)).json()
		anu = anu.images
		...
		for (let x of anu) {
			if (c == 0) await conn.sendMsg(m.chat, { image: { url: x.url }, caption: `Mengirim 1 dari ${anu.length} slide gambar.\n_(Sisanya akan dikirim via chat pribadi.)_` }, { quoted : m })
			else await conn.sendMsg(m.sender, { image: { url: x.url } })
			c += 1
