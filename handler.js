 ...
 */
export async function handler(chatUpdate, client) {
	const msgqueque = new Queque()
	if (!chatUpdate)
		return
	...
	try {
		...
		try {
			...
			if (typeof games !== 'object') db.data.games = {}
			if (games) {
				if (!('akinator' in games)) games.akinator = {}
			} else db.data.games = {
				akinator: {}
			}
			...
			if (datas) {
                ...
                if (!('teksmla' in datas)) datas.teksmla = ''
				if (!('linkgc' in datas)) datas.linkgc = ''
				...
				if (!('panel' in datas)) datas.panel = {}
				if (!('changelog' in datas)) datas.changelog = []
				if (!('request' in datas)) datas.request = []
				if (!('diffusion' in datas)) datas.diffusion = {}
				if (!('request' in datas)) datas.request = {}
				if (!('service' in datas)) datas.service = []
			} else db.data.datas = {
				packname: '',
				author: '',
				linkgc: '',
